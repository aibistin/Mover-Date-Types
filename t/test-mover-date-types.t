...
#-------------------------------------------------------------------------------
my (
    $test_mover_datetime, $test_date_extraction_types,
...
my @good_year_first_strings = (qw / 1945-6-4 2011\/12\/25 2013-7-22 /);

my @maybe_good_year_first_strings =
  ( " the date is 2003/12/11 \n", " in the year of 2005-12-21 " );

my @good_day_first_strings = (qw / 4-6-1946 25\/12\/2011 22-7-2013 /);

my @maybe_good_day_first_strings = ( "the date is 01/07/1999 ",
    " lots of stuff 21-05-1945    and other stuff " );

my @good_month_first_strings = (qw / 6-4-1947 12\/25\/2011 7-22-2011 /);
...
my @maybe_good_iso_date_time_strings =
  ( "the time is 1948-06-06 08:30:01  and counting", " 23:15:10 is the time" );

my @maybe_good_past_date_times = (
    qw / 4-6-1949 25-12-2011 friday yesterday /,
    "last year",
    "last friday",
    "july 31,  1984",
    "three days ago",
...
subtest $test_mover_datetime => sub {
    ...
    ok(
        to_MoverDateTime($ControlDateTime),
...
subtest $test_date_extraction_types => sub {
    ...
    for my $test_string (@good_year_first_strings) {
        is( is_MoverDateStrYearFirst($test_string),
            $TRUE,
            $test_string . ' conains a valid yyyymmdd formatted date string.' );
    ...
    for my $test_string (@good_month_first_strings) {
        is( is_MoverDateStrMonthFirst($test_string),
            $TRUE,
            $test_string . ' conains a valid mmddyyyy formatted date string.' );
...
' to_MoverMonthName DateTime Object worked.try again with one I prepared earlier. '
    ...
    for my $test_month_abbr (qw/feb mar D /) {
        is_any( to_MoverMonthName($test_month_abbr) // $EMPTY,
            \@ctl_month_names_long,
    ...
    for my $test_month (qw / december august 13 /) {
        isnt( is_MoverMonthNameShort($test_month) // $EMPTY,
    ...
    for my $test_month_number ( 1, 6, 12 ) {
        is_any(
            to_MoverMonthNameShort($test_month_number) // $EMPTY,
    ...
    for my $test_month_abbr (qw/ja f mar De/) {
        is_any( to_MoverMonthNameShort($test_month_abbr) // $EMPTY,
            \@ctl_month_names_short,
...
subtest $mover_date_time_wildcard_coersion => sub {
    ...
    for my $good_date_type ( keys %good_test_dates ) {
        ...
        if ( blessed($GotDt) ) {
            ...
            #------ If 1 is passed as a 'Good Date' (Manip converts it to
            #       DateTime->now)
            if ( $good_date_type == 1 ) {
                is( blessed($GotDt), 'DateTime',
                    ' 1 returned a DateTime->now Object' );
            }
            else {
...
'Coerced DateTime from various input contains correct date values.'
            ...
            fail('to_MoverDateTime must return a DateTime Object!');
        }
    }
    ...
    for my $date_href (@good_hash_dates) {
        my $GotDt = undef;
        isa_ok(
            $GotDt = to_MoverDateTime($date_href),
            'DateTime',
            'Coerced to '
              . ( ( $GotDt // $EMPTY ) . "" )
              . ' MoverDateTime from '
...
sub is_datetime_eq_array {
    ...
    $info_msg .=
        "\n Got DateTime: "
      . convert_datetime_to_str($GotDt)
      . "\n Expected DateTime:  "
      . convert_datetime_aref_to_str($expected_array_ref);
...
#-------------------------------------------------------------------------------
#  Convert DateTime ArrayRef to String
#  ArrayFormat :  [year, month, day, hour, minute, second]
#  Returns year/month/day T hh::mm::ss
#-------------------------------------------------------------------------------
sub convert_datetime_aref_to_str {
    my $aref = shift;
    return fail('Must send an ArrayRef to convert_datetime_aref_to_str !')
